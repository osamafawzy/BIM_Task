                    ...
                    <form class="dt_adv_search">
                        <div class="row g-1 mb-md-1">
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="for" class="form-select" id="name511">
                                    <option  selected value="{{ null }}">الكل</option>
                                    <option value="administrative" @if(@$_GET['for']=="administrative") selected @endif>اداري</option>
                                    <option value="teacher" @if(@$_GET['for']=="teacher") selected @endif>معلم</option>
                                </select>
                            </div>
...
@section('js')
    ...
    @if (session('updated'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم تعديل  المستوي/ الرنبة بنجاح',
                icon: 'success',
                customClass: {
    ...
    @if (session('created'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم انشاء المستوي /الرتبة  بنجاح',
                icon: 'success',
                customClass: {
                    confirmButton: 'btn btn-primary'
                },
    ...
    <script>
        $(function() {
            ...
            var type = url.searchParams.get("for")
           
            var ajaxRequest = "teacher_levels?";
            if (type != null) {
                ajaxRequest += "for=" + type + "&";
            }
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return (
                                    ...
                                    '</div>' +
                                    '</div>'
                                    @can('Edit-teacher_levels')
                                        +
                                        '<a href="teacher_levels/' + data +
                        ...
                        @can('Create-teacher_levels')
...

                            {
                                ...
                                action: function(e, dt, node, config) {
                                    ...
                                    window.location.href = './teacher_levels/create';
                                },
                                init: function(api, node, config) {
                                    $(node).removeClass('btn-secondary');
                                }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: window.location.origin + "/admin/teacher_levels/" + id,
                            type: 'POST',
                            data: {
                                "id": id,
                                "_method": "DELETE",

        ...
        <div class="card">
            <div
                class="
                    ...
                    ">
                <div>
                    <h4 class="card-title mb-25"> فلترة المناوبة
                    </h4>
                ...
                <div class="card-body">
                    <form class="dt_adv_search">
                        <div class="row g-1 mb-md-1">
                            ...
                            <div class="col-md-4">
                                <label class="form-label" for="basic-icon-default-date"> من</label>
                                <input type="date" value="{{ @$_GET['date_from'] }}" name="date_from" id="fp-date-time"
                                    class="form-control" />
                            </div>

                            <div class="col-md-4">
                                <label class="form-label" for="basic-icon-default-date"> اللي</label>
                                <input type="date" value="{{ @$_GET['date_to'] }}" name="date_to" id="fp-date-time"
                                    class="form-control" />
                            </div>
...
@section('js')
    ...
    @if (session('created'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم انشاء البيانات بنجاح',
                icon: 'success',
                customClass: {
                    confirmButton: 'btn btn-primary'
                },
    ...
    <script>
        $(function() {
            ...
            var date_from = url2.searchParams.get("date_from")
            var date_to = url2.searchParams.get("date_to")
            var schoolId = url2.searchParams.get("school_id")
            var empId = url2.searchParams.get("employee_id")



            var ajaxRequest = "shift?";

            if (date_from != null) {
                ajaxRequest += "date_from=" + date_from + "&";
            }
            if (date_to != null) {
                ajaxRequest += "date_to=" + date_to + "&";
            }
            if (empId != null) {
                ajaxRequest += "employee_id=" + empId + "&";
            }
            if (schoolId != null) {
                ajaxRequest += "school_id=" + schoolId + "&";
            }
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ajax: '/admin/school/' + school_id + '/' + ajaxRequest,
                    columns: [{
                            data: 'id'
                        }, // for responsive show
                        {
                        ...
                        @can('Download-shift')
                            {
                                ...
                                action: function(e, dt, node, config) {
                                    //This will send the page to the location specified
                                    window.location.href = '/admin/school/' + school_id +
                                        '/shift/download?date_from='+date_from+'&date_to='+date_to;
                                },
                                init: function(api, node, config) {
                                    $(node).removeClass('btn-secondary');
                                }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: window.location.origin + "/admin/school/" + school_id +
                                "/shift/" + id,
                            type: 'POST',
                            data: {
                                "id": id,
